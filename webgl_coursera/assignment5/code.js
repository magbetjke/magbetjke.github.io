...
function Drawable(type, tessellationLevel) {
    ...
    this.addTriangle = function (a, b, c) {
        ...
        if (this.type == "sphere") {
            function add(vertex) {
                var normal = normalize(subtract(vec3(0, 0, 0), vertex));
                this.normals.push(normal);

                this.uv.push([Math.atan2(normal[1], normal[2]) / Math.PI, Math.acos(-normal[0]) / Math.PI]);
                this.planarUV.push([normal[0], normal[1]]);
            }
...
//        gl.uniform4fv(lightpos_loc, directionalLight.position);
        ...
        gl.uniform1i(gl.getUniformLocation(lightProgram, "texture"), 0);
        ...
        if (currentMapping == "planar" || currentMapping == "planar_image") {
            gl.bufferSubData(gl.ARRAY_BUFFER, 0, flatten(this.planarUV));
        } else {
            gl.bufferSubData(gl.ARRAY_BUFFER, 0, flatten(this.uv));
        }
...
function setMapping(mapping) {
    ...
    if (currentMapping == "planar_image" || currentMapping == "spherical_image") {
...
//    gl.clearColor(0.98, 0.98, 0.98, 1.0);
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexBufferId);
    gl.bufferData(gl.ARRAY_BUFFER, 16 * 2048 * 32, gl.DYNAMIC_DRAW);
    ...
    var vTexCoord = gl.getAttribLocation(lightProgram, "vTexCoord");
    ...
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, indexBufferId);
    gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, 16 * 2048 * 32, gl.DYNAMIC_DRAW);

    createDefaultTexture(128, 10);
...
//      		checkHit(event.clientX, event.clientY);
    ...
    canvas.addEventListener("mousemove", function(event){
        if (mousePressed)
        {
            ...
            selectedObject.rotation[1] -= prevMouseX - currentX;
    ...
    canvas.addEventListener("mousewheel", function(event){
        ...
        if (selectedObject.scale[1] < 0) {
            selectedObject.scale[1] = 0;
        }
        if (selectedObject.scale[2] < 0) {
            selectedObject.scale[2] = 0;
        }
...
//    };
    ...
    document.getElementById("mapping_list").onchange = function (event) {
        var target = event.target || event.srcElement;
        var mapping = target.value;
        setMapping(mapping);
    };
...
//    handleTextureLoaded(image, earthTexture);
...
//    drawable.push(cylinder);
    ...
    var sphere = new Drawable("sphere", 5);
    sphere.scale = [2, 2, 2];
    sphere.position = [0, 0, 0];
    sphere.color = [1.0, 0.0, 0.0, 1.0];
    sphere.rotation = [255, 112, 5];
...
function createDefaultTexture(texSize, numChecks) {
    defaultTexture = new Uint8Array(3*texSize*texSize);
    for ( var i = 0; i < texSize; i++ ) {
        for ( var j = 0; j <texSize; j++ ) {
            ...
            defaultTexture[3*i*texSize+3*j] = c;
            defaultTexture[3*i*texSize+3*j+1] = c;
            defaultTexture[3*i*texSize+3*j+2] = c;
        }
    }
    ...
    gl.texImage2D( gl.TEXTURE_2D, 0, gl.RGB, texSize, texSize, 0, gl.RGB, gl.UNSIGNED_BYTE, defaultTexture );
